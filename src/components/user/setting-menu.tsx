...
export default function SettingMenu() {
    ...
    const generateHref = (path: string) => `/${locale}/my-account/${session?.user.id}${path}`;
    ...
    const links = [
        ...
        { path: "#", label: t("deleteAccount"), className: "text-red-600 underline" },
    ];
        ...
        <nav className="flex flex-col my-5 gap-4 text-xl">
            {links.map(({ path, label, className = "" }) => (
                <Link
                    ...
                    className={`font-medium ${pathname === generateHref(path) ? "text-red-500 underline duration-100" : ""} ${className}`}
                >
                    {label}
                </Link>
            ))}
