...
export default function RegisterForm() {
    ...
    return (
        <>
            ...
            <form className="flex flex-col gap-5 w-5/6  md:w-4/6 xl:w-3/6" onSubmit={handleSubmit(onSubmit)}>
                <div>
                    <Input type="text" {...register("firstName")} placeholder={t("placeholderFirstName")}/>
                    {errors.firstName && (
                ...
                <div>
                    <Input type="text" {...register("lastName")} placeholder={t("placeholderLastName")}/>
                    {errors.lastName && (
                        <span className="text-red-500 text-xs pt-1 block">
     {t(`${errors.lastName?.message as string}`)}
                ...
                <div>
                    <Input type="email" {...register("email")} placeholder={t("placeholderEmail")}/>
                    {errors.email && (
                        <span className="text-red-500 text-xs pt-1 block">
  ...
  {t(`${errors.password?.message as string}`)}
    ...
    {t(`${errors.passwordConfirm?.message as string}`)}
             ...
             <LoadingButton/>
                ...
                <Separator/>
                <p className="text-center">{t("alreadyAccount")} <Link className="underline " href={redirect}>
                    {t("login")}
                </Link></p>
            </form>
        </>
