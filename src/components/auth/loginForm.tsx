...
export const LoginForm = () => {
    ...
    return (
        <>
            ...
            <h2 className="text-balance text-muted-foreground">{t("subtitleLogin")}</h2>
            <form className="flex flex-col gap-5 w-5/6  md:w-4/6 xl:w-3/6" onSubmit={handleSubmit(onSubmitHandler)}>
                <div>
                    <Input
                        type='email'
                        {...register("email")}
               ...
               {t(`${errors.email?.message as string}`)}
                ...
                <div>
                    ...
                    />
                    <Link href={"forgot-password"} className="text-xs text-primary">{t("forgotPassword")}</Link>
                    {errors.password && (
                        <span className='text-red-500 text-xs pt-1 block'>
